	...
	go func() {
		for {
			if intentDelay {
				...
				// Check state of VM.
				state, _ := me.BoxInstance.GetCachedState()
				me.SetMenuState(menu, state)
				...
				// Check state of VM.
				state, _ := me.BoxInstance.GetState()
				me.SetMenuState(menu, state)
...
func (me *Heartbeat) SetMenuState(menu menuStruct, state box.BoxState) (returnValue int) {
