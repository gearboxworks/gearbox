...
func Debug(client msgs.Address, format string, a ...interface{}) {
...
func LogIfError(address msgs.Address, err error, format ...interface{}) bool {
...
func NewLogger(OsPaths *osdirs.BaseDirs, args ...Logger) (*Logger, error) {
	...
	for range only.Once {
		...
		if _args.EntityId == nil {
			_args.EntityId = &_args.EntityName // msg.MakeAddress()
		}

		_args.OsPaths = osdirs.New()
		//_args.baseDir = _args.osPaths.UserConfigDir.AddPaths(DefaultBaseDir)
		//_args.pidFile = osdirs.AddFilef(_args.baseDir,DefaultPidFile).String()
