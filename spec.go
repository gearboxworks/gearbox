...
func (me *Spec) String() string {
	var s string
	if me.Host == "" && me.Namespace == "" && me.Version == "" {
		s = me.Role
	} else if me.Host == "" && me.Version == "" {
		s = fmt.Sprintf("%s/%s", me.Namespace, me.Role)
	} else if me.Host == "" && me.Namespace == "" {
		s = fmt.Sprintf("%s:%s", me.Role, me.Version)
	} else if me.Host == "" {
		s = fmt.Sprintf("%s/%s:%s", me.Namespace, me.Role, me.Version)
	} else if me.Version == "" {
		s = fmt.Sprintf("%s/%s/%s", me.Host, me.Namespace, me.Role)
	} else {
		s = fmt.Sprintf("%s/%s/%s:%s", me.Host, me.Namespace, me.Role, me.Version)
	}
	return s
}
