import Vue from 'vue'
import { BaseMutations } from './method-names'
import { namespaceValues } from '../../_helpers'

export function NamespacedBaseMutations (api, namespace) {
  ...
  return {
    ...
    [Mutations.SET_ONE] (state, record) {
      ...
      if (!record.attributes || Object.entries(record.attributes).length === 0) {
        throw new Error(
          'An record with no attributes is invalid and cannot be put to store: ' +
          record.id)
      }
    ...
    [Mutations.UPDATE] (state, payload) {
      ...
      Vue.set(record, 'attributes',
        { ...record.attributes, ...recordData.attributes })
    },
...
export default NamespacedBaseMutations
